...
import { DashboardMetrics, TopEndpoint } from '@/app/core/services/dashboard-service';
    ...
    template: `
        <div class="card h-full">
            ...
            <ul class="list-none p-0 m-0">
                <li *ngFor="let endpoint of topEndpoints; let i = index" class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                    <div>
                        <span class="text-surface-900 dark:text-surface-0 font-medium mr-2 mb-1 md:mb-0">{{ endpoint.endpoint_name }}</span>
                        <div class="mt-1 text-muted-color">ID: {{ endpoint.endpoint_id }}</div>
                    </div>
                    <div class="mt-2 md:mt-0 flex items-center">
                        <div class="bg-surface-300 dark:bg-surface-500 rounded-border overflow-hidden w-40 lg:w-24" style="height: 8px">
                            <div [class]="getBarClass(i)" style="height: 100%;" [style.width]="getPercentage(endpoint.total_hits) + '%'"></div>
                        </div>
