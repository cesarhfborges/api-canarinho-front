...
@Component({
    ...
    imports: [TableModule, ButtonModule, NgClass, JsonPipe],
...
export class EndpointVisualizar implements OnInit {
    ...
    async carregarDados(event?: any) {
        ...
        if (!username || !projectSlug || !endpointName || !token) {
            this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Faltam parâmetros para carregar os dados.', life: 3000 });
        ...
        this.http.get<any>(url, { headers }).subscribe({
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Falha ao buscar os dados da API.', life: 3000 });
                this.loading.set(false);
            }
        });
    }
